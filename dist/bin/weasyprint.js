...
program
    ...
    const fetcher = new URLFetcher({
        ...
        allowedProtocols: opts['allowedProtocols']
            ? new Set(String(opts['allowedProtocols']).split(',').map((s) => s.trim().toLowerCase()))
            : undefined,
        allowRedirects: !opts.noHttpRedirects,
        failOnErrors: Boolean(opts.failOnHttpErrors),
    });
    let source;
    if (input === '-') {
        const chunks = [];
    ...
    const baseUrl = opts['baseUrl'] ?? (input === '-' ? null : null);
    const html = await HTML.create((opts['encoding'] ? { filename: String(source) } : source), { baseUrl: baseUrl ?? null, urlFetcher: fetcher, mediaType: String(opts['mediaType'] ?? 'print') });
    const stylesheets = [];
    const extra = opts['stylesheet'] ?? [];
    ...
    const pdf = await html.writePdf(null, {
        ...
        attachments: await loadAttachments(opts['attachment'] ?? []),
...
//# sourceMappingURL=weasyprint.js.map