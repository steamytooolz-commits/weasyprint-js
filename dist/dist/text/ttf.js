...
const registry = [];
export function registerFont(buffer) {
    try {
...
export function matchFont(families, weight, italic) {
...
export function measureParsed(parsed, text, fontSizePx) {
    let units = 0;
    for (const ch of text)
        units += glyphAdvance(parsed, ch);
...
function parseTtf(buf) {
    ...
    const i16 = (o) => view.getInt16(o, false);
    if (buf.length < 12)
        return null;
    const sfnt = u32(0);
    ...
    for (let gid = n; gid < numGlyphs; gid++)
        advances.set(gid, lastAdvance);
...
function parseCmap(view, buf, offset) {
    const out = new Map();
    const u16 = (o) => view.getUint16(o, false);
    const u32 = (o) => view.getUint32(o, false);
    ...
    if (best12 >= 0)
        parseCmap12(view, buf, best12, out);
    else if (best4 >= 0)
        parseCmap4(view, buf, best4, out);
    return out;
}
function parseCmap12(view, buf, offset, out) {
    const u32 = (o) => view.getUint32(o, false);
    ...
    for (let i = 0; i < numGroups; i++) {
        ...
        for (let c = start; c <= end && c - start < 4096; c++) {
            if (!out.has(c))
                out.set(c, glyph + (c - start));
        }
    }
}
function parseName(view, buf, table) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        void encoding;
        if ((nameId === 1 || nameId === 16) && !family)
            family = text;
        if ((nameId === 2 || nameId === 17) && !subfamily)
            subfamily = text;
    }
    return { family: family.replace(/\0/g, ''), subfamily: subfamily.replace(/\0/g, '') };
}
function parseCmap4(view, buf, offset, out) {
    __parseCmap4ForTest(view, buf, offset, out);
}
// Re-export a real format-4 parser (kept separate for clarity/testing).
export function __parseCmap4ForTest(view, buf, offset, out) {
    const u16 = (o) => view.getUint16(o, false);
    if (offset + 14 > buf.length)
        return;
    const segCount = u16(offset + 6) / 2;
    if (!Number.isFinite(segCount) || segCount <= 0 || segCount > 256)
        return;
