...
function parentFontPx(parentStyle) {
    const raw = (parentStyle['font-size'] ?? '16px').trim().toLowerCase();
    const parsed = parseLength(raw);
    if (!parsed) {
        if (ABSOLUTE_FONT_PX[raw] != null)
            return ABSOLUTE_FONT_PX[raw];
        return 16;
    }
    ...
    if (px != null && Number.isFinite(px) && px > 0)
        return px;
    if (parsed.unit === '%')
        return (16 * parsed.value) / 100;
    return 16;
}
function canonicalPx(num, unit, fontPx) {
...
function canonicalLength(value, fontPx) {
    const parsed = parseLength(value.trim());
    if (!parsed)
        return null;
    if (parsed.unit === '%')
        return null; // percentages preserved by caller
    if (parsed.unit === null) {
        if (parsed.value === 0)
            return '0px';
        return null;
...
function computeFontSize(value, parentStyle) {
    const v = value.trim().toLowerCase();
    const parent = parentFontPx(parentStyle);
    if (ABSOLUTE_FONT_PX[v] != null)
        return `${ABSOLUTE_FONT_PX[v]}px`;
    if (v === 'smaller')
        return `${Math.round(parent * 0.8 * 10000) / 10000}px`;
    if (v === 'larger')
        return `${Math.round(parent * 1.2 * 10000) / 10000}px`;
    const parsed = parseLength(value.trim());
    if (!parsed)
        return value.trim();
    if (parsed.unit === '%') {
...
function computeLineHeight(value, fontPx) {
    const v = value.trim().toLowerCase();
    if (v === 'normal')
        return 'normal';
    if (/^[+-]?(\d*\.?\d+)$/.test(v))
        return v.replace(/^\+/, '');
    if (v.endsWith('%'))
        return v; // percentages preserved
    return canonicalLength(value, fontPx) ?? value.trim();
}
function computeFontWeight(value, parentStyle) {
    const v = value.trim().toLowerCase();
    if (v === 'normal')
        return '400';
    if (v === 'bold')
...
export function computeValue(prop, value, parentStyle) {
    ...
    if (p === 'font-size')
        return computeFontSize(v, parentStyle);
    if (p === 'line-height') {
    ...
    if (p === 'font-weight')
        return computeFontWeight(v, parentStyle);
    if (vl === 'currentcolor') {
        if (p === 'color')
            return (parentStyle['color'] ?? 'black').trim() || 'black';
        return (parentStyle['color'] ?? 'black').trim() || 'black';
    }
    if (vl.endsWith('%') && /^-?(\d*\.?\d+)%$/.test(vl))
        return vl;
    if (LENGTH_PROPS.has(p)) {
        if (/^(auto|normal|none|min-content|max-content|fit-content|content)$/.test(vl))
            return vl;
        const parent = parentFontPx(parentStyle);
        ...
        if (/calc\(|min\(|max\(|clamp\(/i.test(v) && !v.includes('%')) {
            ...
            if (px != null && Number.isFinite(px))
                return `${Math.round(px * 10000) / 10000}px`;
        }
    ...
    if (p === 'color' || p.endsWith('-color'))
        return vl;
    return v;
}
//# sourceMappingURL=computed-values.js.map