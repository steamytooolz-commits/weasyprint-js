 ...
 */
export function evaluateLengthToPx(raw, ctx = {}, vars = {}) {
    let value = raw.trim();
    if (!value)
        return null;
    ...
    if (low === 'auto' || low === 'none' || low === 'normal' || low === 'content')
        return null;
    ...
    try {
        if (fn === 'calc')
            return evalSum(body, ctx, vars);
        if (fn === 'clamp')
            return evaluateClamp(value, ctx, vars);
...
export function evaluateClamp(raw, ctx = {}, vars = {}) {
    const m = /^clamp\(/i.exec(raw.trim());
    if (!m || !raw.trim().endsWith(')'))
        return null;
...
function evalSum(expr, ctx, vars) {
    // Tokenize on top-level + / - (* and / per spec; support them too).
    const tokens = [];
    let depth = 0;
    let cur = '';
    const push = () => { if (cur.trim())
    ...
    const vals = tokens.map((t) => {
        ...
        if (Number.isFinite(f) && /^[-+]?(\d*\.?\d+)$/.test(t.trim()))
            return f;
        return NaN;
    });
    // * and / first.
    ...
    for (let i = 0; i < vals.length; i++) {
        ...
        if (v === '*' || v === '/') {
            ...
            if (typeof a !== 'number' || typeof b !== 'number' || Number.isNaN(a) || Number.isNaN(b))
                return null;
            if (v === '/' && b === 0)
                return null; // spec: division by zero → infinity; fail safe
...
function lengthToPx(token, ctx) {
    ...
    if (u === 'em')
        return n * (ctx.fontPx ?? 16);
    if (u === 'rem')
        return n * (ctx.rootPx ?? 16);
    if (u === 'ex' || u === 'ch' || u === 'cap')
        return n * (ctx.fontPx ?? 16) * 0.5;
    if (u === 'lh')
        return n * (ctx.fontPx ?? 16) * 1.2;
    if (u === 'vw' || u === 'vh' || u === 'vmin' || u === 'vmax')
        return null; // no viewport in print
...
export function resolveVars(value, vars) {
    ...
    for (let guard = 0; guard < 10; guard++) {
        ...
        if (replacement == null)
            return null; // guaranteed-invalid
