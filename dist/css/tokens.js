...
let orderCounter = 0;
export function parseStylesheet(css, media = null) {
    const rules = [];
    if (!css.trim())
        return { rules };
    let ast;
    ...
    walk(ast, (node) => {
        if (node.type === 'Rule') {
            ...
            const selectors = splitSelectors(generate(prelude));
            const declarations = [];
            for (const decl of block?.children?.toArray?.() ?? []) {
                if (!decl.property || decl.value == null)
                    continue;
                declarations.push({
            ...
            rules.push({
                type: 'at',
                name,
                prelude: node['prelude'] ? generate(node['prelude']).trim() : '',
                block: node['block'] ? generate(node['block']) : null,
                media,
                order: orderCounter++,
            });
        }
...
export function expandShorthands(decls) {
    const out = [];
    for (const d of decls) {
        if (d.prop === 'margin' || d.prop === 'padding') {
            const parts = d.value.split(/\s+/).filter(Boolean);
            const [t, r, b, l] = quad(parts);
            out.push({ prop: `${d.prop}-top`, value: t, important: d.important }, { prop: `${d.prop}-right`, value: r, important: d.important }, { prop: `${d.prop}-bottom`, value: b, important: d.important }, { prop: `${d.prop}-left`, value: l, important: d.important });
        }
        else if (d.prop === 'border-width' || d.prop === 'border-style' || d.prop === 'border-color') {
            const parts = d.value.split(/\s+/).filter(Boolean);
            const [t, r, b, l] = quad(parts);
            const kind = d.prop.slice('border-'.length);
            out.push({ prop: `border-top-${kind}`, value: t, important: d.important }, { prop: `border-right-${kind}`, value: r, important: d.important }, { prop: `border-bottom-${kind}`, value: b, important: d.important }, { prop: `border-left-${kind}`, value: l, important: d.important });
        }
        else if (d.prop === 'font') {
            for (const e of expandFont(d.value, d.important))
                out.push(e);
        }
        else {
            out.push(d);
...
function expandFont(value, important) {
    ...
    if (!m)
        return out;
    if (m[1] && m[1] !== 'normal')
        out.push({ prop: 'font-style', value: m[1], important });
    if (m[2] && m[2] !== 'normal')
        out.push({ prop: 'font-weight', value: m[2], important });
    out.push({ prop: 'font-size', value: m[3], important });
    if (m[4])
        out.push({ prop: 'line-height', value: m[4], important });
