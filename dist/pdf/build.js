...
import { PDFDocument, StandardFonts, rgb, PDFName, PDFString, PDFHexString, PDFNull, pushGraphicsState, popGraphicsState, moveTo, lineTo, closePath, clip, endPath, } from 'pdf-lib';
...
export async function buildPdf(pageBox, flow, opts = {}) {
    ...
    if (opts.withMetadata !== false) {
        ...
        if (opts.keywords)
            doc.setKeywords(Array.isArray(opts.keywords) ? opts.keywords : [opts.keywords]);
    ...
    const getFont = async (name) => {
        ...
        if (!f) {
            f = await doc.embedFont(mapFont(name));
            fonts.set(name, f);
        }
        return f;
    };
    ...
    const ensureFontkit = async () => {
        ...
        fontkitTried = true;
        try {
            const load = Function('return import("@pdf-lib/fontkit")');
    ...
    const resolvePdfFont = async (famSpec, weight, italic) => {
    ...
    const embedRaster = async (buffer, mime) => {
    ...
    const pageIndexAt = (y) => Math.min(pageCount - 1, Math.max(0, Math.floor(y / contentH)));
    const pageTopOf = (idx) => idx * contentH;
    // Anchor map: id/name -> { pageIndex, yPx } for internal `#fragment` links.
    const anchors = new Map();
    for (const block of flow.blocks) {
        const id = block.box.attrs['id'];
        if (id)
            anchors.set(id, { pageIndex: pageIndexAt(block.y), yPx: block.y });
    ...
    const drawBlock = async (block) => {
        ...
        for (const line of block.lines) {
            if (!line.text.trim() && block.box.tag !== 'a')
                continue;
            ...
            if (lsPx !== 0) {
                ...
                for (const ch of outText) {
                    ...
                    try {
                        cx += font.widthOfTextAtSize(ch, fsPt) + stepPt;
                    }
                    catch {
                        cx += fsPt * 0.5 + stepPt;
                    }
            ...
            if (href && outText.trim()) {
                ...
                const rect = [xPt, yPt - fsPt * 0.2, xPt + Math.max(4, wPt), yPt + hPt * 0.9];
                if (href.startsWith('#')) {
                    const id = href.slice(1);
                    if (id)
                        pendingLinks.push({ kind: 'internal', id, rect, pageIndex: li });
                }
    ...
    const drawImage = async (block, page, pageTop, opacity) => {
    ...
    const drawBoxShadow = (block, opacity) => {
        const shadow = parseBoxShadow(block.box.style);
        if (!shadow || block.height <= 0)
            return;
        const rw = pxToPt(pageBox.contentWidth);
        let remaining = block.height;
        let by = block.y;
    ...
    const drawBackgroundImage = async (block, opacity) => {
        const bg = block.box.bgImage;
        if (!bg || block.height <= 0)
            return;
        if (bg.mime !== 'image/png' && bg.mime !== 'image/jpeg')
            return;
        const style = block.box.style;
        const repeat = (style['background-repeat'] ?? 'repeat').trim().toLowerCase().split(/\s+/)[0] ?? 'repeat';
        const { tw, th } = bgTileSize(style['background-size'] ?? 'auto', Math.max(1, pageBox.contentWidth), Math.max(1, block.height), bg.wPx, bg.hPx);
        if (tw <= 0 || th <= 0)
            return;
        const { ox, oy } = bgOrigin(style['background-position'] ?? '0% 0%', Math.max(1, pageBox.contentWidth), Math.max(1, block.height), tw, th);
    ...
    const drawListMarker = (block, page, pageTop) => {
        const first = block.lines[0];
        const style = block.box.style;
        // Real counter snapshots win; flow-order counting is the fallback.
        const snap = block.box.listMarker;
        const type = (snap?.type ?? style['list-style-type'] ?? 'disc').trim().toLowerCase();
    ...
    const drawBordersForSegmentOn = (pg, segPageTop, style, segY, segH, isFirst, isLast, opacity = 1) => {
        const widths = borderWidths(style);
        if (widths.top + widths.right + widths.bottom + widths.left <= 0)
            return;
        const rx = ml;
        const rw = pxToPt(pageBox.contentWidth);
    ...
    const drawBordersClippedOn = (pg, pgIndex, style, top, height, opacity = 1) => {
        if (height <= 0) {
        ...
        while (remaining > 0) {
            ...
            const pTop = pageTopOf(idx);
            const onPage = Math.min(remaining, pTop + contentH - by);
            drawBordersForSegmentOn(target, pTop, style, by, onPage, by === top, by + onPage >= top + height, opacity);
            remaining -= onPage;
            by += onPage;
        }
    };
    ...
    if (opts.attachments) {
        for (const att of opts.attachments) {
            try {
                await doc.attach(att.source, att.name);
            }
            catch (e) {
...
function letterSpacingPx(style, fontPx) {
...
async function drawMarginBoxes(doc, pages, pageBox, resolveFont) {
    const defs = pageBox.marginBoxes ?? {};
    const names = Object.keys(defs).filter((n) => MARGIN_BOX_NAMES.includes(n));
    if (names.length === 0)
        return;
    const pageW = pxToPt(pageBox.widthPx);
    const pageH = pxToPt(pageBox.heightPx);
    ...
    for (let p = 0; p < pages.length; p++) {
        ...
        for (const name of names) {
            ...
            const align = (style['text-align'] ??
                (name.includes('center') || name.includes('middle') ? 'center' : name.includes('right') ? 'right' : 'left')).trim().toLowerCase();
            let wPt = fsPt * text.length * 0.5;
            try {
                wPt = font.widthOfTextAtSize(text, fsPt);
            }
            catch { /* keep estimate */ }
            let x = rect.x;
            if (align === 'center')
                x = rect.x + Math.max(0, (rect.w - wPt) / 2);
            else if (align === 'right' || align === 'end')
                x = rect.x + Math.max(0, rect.w - wPt);
            // Vertical: top row → top, middle row → centered, bottom row → baseline up.
            let y;
            if (name.startsWith('top'))
                y = rect.yTop - fsPt * 1.0;
            else if (name.startsWith('bottom'))
                y = rect.yTop - rect.h + fsPt * 0.25;
            else
                y = rect.yTop - rect.h / 2 - fsPt * 0.35;
            page.drawText(text, { x, y, size: fsPt, font, color: rgb(col.r, col.g, col.b), opacity });
        }
    }
}
/** Margin-box rectangle in PDF points: { x, yTop, w, h }. */
function marginRect(name, pageW, pageH, mT, mB, mL, mR, midW, midH) {
    const thirdW = midW / 3;
    const thirdH = midH / 3;
    switch (name) {
...
function addLinkAnnotations(doc, pages, links, anchors, pageH, mt, contentH) {
    if (links.length === 0)
        return;
...
async function addOutlines(doc, pages, bookmarks, uidPos, pageH, mt, contentH) {
    ...
    try {
        ...
        for (const it of items) {
            const node = { title: it.title, pageIndex: it.pageIndex, yPx: it.yPx, children: [] };
        ...
        const refs = [];
        const flat = [];
        const walkFlat = (n) => { flat.push(n); for (const c of n.children)
            walkFlat(c); };
        for (const r of roots)
            walkFlat(r);
        const refByNode = new Map();
        for (const n of flat)
            refByNode.set(n, ctx.nextRef());
        const countOf = (n) => {
        ...
        for (const e of refs) {
            ...
            if (e.nextRef)
                dict['Next'] = e.nextRef;
            if (e.firstRef) {
                dict['First'] = e.firstRef;
                dict['Last'] = e.lastRef;
                dict['Count'] = e.count;
            }
            ctx.assign(e.ref, ctx.obj(dict));
        }
        const outlinesDict = ctx.obj({
            Type: PDFName.of('Outlines'),
            First: refByNode.get(roots[0]),
            Last: refByNode.get(roots[roots.length - 1]),
            Count: flat.length,
        });
...
function withClip(page, x, y, w, h, fn) {
...
function bgTileSize(size, areaW, areaH, natW, natH) {
    const t = size.trim().toLowerCase();
    if (t === '' || t === 'auto')
        return { tw: natW, th: natH };
...
function bgOrigin(pos, areaW, areaH, tw, th) {
...
function bgPosAxis(token, area, tile, isX) {
    const kw = isX
        ? { left: 0, center: 0.5, right: 1 }
        : { top: 0, center: 0.5, bottom: 1 };
    if (token in kw)
        return (area - tile) * kw[token];
    // center is shared
    if (token === 'center')
        return (area - tile) * 0.5;
    if (token.endsWith('%')) {
...
function tileOffsets(tiled, o, area, t) {
    if (!tiled)
        return [o];
    const out = [];
    ...
    for (let x = start; x < area; x += t) {
        out.push(x);
        if (out.length > 1000)
            break; // degenerate guard
    }
    return out.length > 0 ? out : [o];
}
function markerLabel(type, n) {
