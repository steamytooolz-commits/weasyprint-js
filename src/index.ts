...
import { selectSource, type SourceInput } from './source.js';
...
import type { FontFaceRule } from './css/types.js';
...
function warnUnknownOptions(opts: Record<string, unknown>, where: string): void {
...
export const DEFAULT_OPTIONS = {
  stylesheets: null as RenderOptions['stylesheets'],
  mediaType: 'print',
  ...
  hinting: false,
  zoom: 1,
  pdfVariant: null as string | null,
  pdfVersion: null as string | null,
  pdfTags: false,
  attachments: null as RenderOptions['attachments'],
};
...
export class HTML {
  ...
  private constructor(html: string, baseUrl: string, opts: { urlFetcher?: URLFetcher; mediaType?: string }) {
  ...
  static async create(input: HtmlInput, opts: { baseUrl?: string | null; urlFetcher?: URLFetcher; mediaType?: string } = {}): Promise<HTML> {
    ...
    if (m) {
      try { baseUrl = new URL(m[1], baseUrl).href; } catch { /* keep */ }
    }
    return new HTML(html, baseUrl, { urlFetcher: fetcher, mediaType: opts.mediaType ?? 'print' });
  }

  async collectCss(extra: Array<CSS | string> = []): Promise<Array<{ css: string; origin: 'author' }>> {
    const out: Array<{ css: string; origin: 'author' }> = [];
      ...
      if (typeof s === 'string') {
        ...
        if (/^(https?|file|data):/.test(s) || s.endsWith('.css')) {
          try {
            ...
            out.push({ css: fetched.buffer.toString('utf8'), origin: 'author' });
            continue;
          } catch { out.push({ css: '', origin: 'author' }); continue; }
        }
  ...
  async render(opts: RenderOptions = {}): Promise<Document> {
    warnUnknownOptions(opts as Record<string, unknown>, 'render');
    ...
    const { matched, pageRules, fontFaces, pseudos } = buildCascade(
      tree.elements,
      sheets.map((s) => ({ css: s.css, origin: s.origin })),
      {
    ...
    const docOpts = { ...(opts as Record<string, unknown>), baseUrl: opts.baseUrl ?? this.baseUrl } as Record<string, unknown>;
    return new Document(pruned, tree.elements, pageBox, flow, docOpts, bookmarks);
  }

  async writePdf(target?: string | null, opts: RenderOptions = {}): Promise<Buffer> {
    const doc = await this.render(opts);
    const merged: RenderOptions = { ...doc.opts, ...opts };
    const bytes = await doc.writePdf(merged);
    const buf = Buffer.from(bytes);
    if (target) await writeFile(target, buf);
    return buf;
  }
}
...
export class CSS {
  cssText: string;
  baseUrl: string | null;
  private constructor(css: string, baseUrl: string | null) {
    this.cssText = css;
    this.baseUrl = baseUrl;
  }
  static async create(input: string | Buffer | URL | SourceInput, opts: { baseUrl?: string | null; urlFetcher?: URLFetcher } = {}): Promise<CSS> {
    const fetcher = opts.urlFetcher ?? new URLFetcher();
    const resolved = await selectSource(input, { baseUrl: opts.baseUrl ?? null, fetcher });
    return new CSS(resolved.buffer.toString('utf8'), resolved.baseUrl);
  }
  static fromString(css: string, baseUrl: string | null = null): CSS {
...
export class Attachment {
  source: Buffer;
  name: string | null;
  constructor(source: Buffer, name: string | null = null) {
    this.source = source;
    this.name = name;
  }
}
...
async function attachDocumentFonts(
  faces: FontFaceRule[], baseUrl: string, fetcher: URLFetcher,
): Promise<void> {
  for (const face of faces) {
    ...
    for (let i = 0; i < urls.length; i++) {
      if (formats[i] && formats[i].includes('woff')) continue;
      const u = urls[i];
      if (!u || u.toLowerCase().startsWith('local(')) continue;
      try {
        let buffer: Buffer;
        if (u.toLowerCase().startsWith('data:')) {
          ...
          let abs = u;
          try { abs = new URL(u, baseUrl).href; } catch { /* keep */ }
...
/** Resolve every <img> in the box tree to bytes + intrinsic size. */async function attachDocumentImages(root: Box, baseUrl: string, fetcher: URLFetcher): Promise<void> {
  const imgs: Box[] = [];
  const walk = (box: Box): void => {
    if (box.tag === 'img') imgs.push(box);
...
async function attachBackgroundImages(root: Box, baseUrl: string, fetcher: URLFetcher): Promise<void> {
  const boxes: Box[] = [];
  const walk = (box: Box): void => {
    if (box.uid !== 0) {
      const bg = box.style['background-image'] ?? '';
      const m = /url\(\s*(['"]?)(.*?)\1\s*\)/i.exec(bg);
      if (m && m[2].trim() && m[2].trim().toLowerCase() !== 'none') boxes.push(box);
    }
    for (const c of box.children) walk(c);
  };
  walk(root);
  ...
  async function worker(): Promise<void> {
    while (i < boxes.length) {
      ...
      if (!src) { box.bgImage = null; continue; }
      try {
        let buffer: Buffer;
        if (src.toLowerCase().startsWith('data:')) {
          const parsed = parseDataUrl(src);
          if (!parsed) { box.bgImage = null; continue; }
          buffer = parsed.data;
        } else {
          let abs = src;
          try { abs = new URL(src, baseUrl).href; } catch { /* keep */ }
          buffer = (await fetcher.fetch(abs)).buffer;
        }
        const info = sniffImage(buffer);
        if (!info || info.mime === 'image/svg+xml') { box.bgImage = null; continue; }
...
function collectBookmarks(root: Box): Bookmark[] {
  const out: Bookmark[] = [];
  const walk = (box: Box): void => {
    const m = /^h([1-6])$/.exec(box.tag);
    if (m && box.uid !== 0) {
      const label = headingText(box).replace(/\s+/g, ' ').trim().slice(0, 200);
      if (label) out.push({ level: parseInt(m[1], 10), label, uid: box.uid });
    }
    for (const c of box.children) walk(c);
  };
  walk(root);
...
function headingText(box: Box): string {
  const parts: string[] = [];
