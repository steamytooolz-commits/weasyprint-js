 ...
 */
export function distributeTableWidths(
  cells: TableCell[][],
  availablePx: number,
  ...
  let nCols = 0;
  for (const row of cells) nCols = Math.max(nCols, row.length);
  if (nCols === 0) return cells.map(() => []);

  const colMin: number[] = new Array(nCols).fill(0);
  const colMax: number[] = new Array(nCols).fill(Infinity);
  const hasCell: boolean[] = new Array(nCols).fill(false);
  ...
  if (sumMin >= forCols) {
    ...
    const frozen: boolean[] = new Array(nCols).fill(false);
    for (let iter = 0; iter < nCols + 1; iter++) {
      let frozenSum = 0;
      let unfrozenCount = 0;
      for (let i = 0; i < nCols; i++) {
        if (frozen[i]) frozenSum += widths[i];
        else unfrozenCount++;
      }
      ...
      for (let i = 0; i < nCols; i++) {
        ...
        if (fair < colMin[i] - 1e-9 || fair > colMax[i] + 1e-9) violating.push(i);
      }
      ...
      for (const i of violating) {
        if (fair < colMin[i]) widths[i] = colMin[i];
        else widths[i] = colMax[i];
        frozen[i] = true;
      }
    }
  }
...
export function tableRowHeights(rows: number[][]): number[] {
  return rows.map((row) => {
