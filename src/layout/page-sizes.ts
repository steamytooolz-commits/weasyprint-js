...
export const ALL_PAGE_SIZES: Record<string, [number, number]> = {
  // ISO A series
  a0: [841, 1189],
  a1: [594, 841],
  a2: [420, 594],
...
export const PAGE_SIZE_NAMES: string[] = Object.keys(ALL_PAGE_SIZES);
...
function parseLengthToMm(token: string): number | null {
 ...
 */
export function parsePageSize(sizeStr: string): ParsedPageSize {
  ...
  for (const tok of tokens) {
    if (tok === 'landscape') landscapeKw = true;
    else if (tok === 'portrait') portraitKw = true;
    else rest.push(tok);
  }
