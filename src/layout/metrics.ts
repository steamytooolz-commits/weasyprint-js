...
export function fontSizePx(style: Record<string, string>, parent = 16): number {
  ...
  if (u === 'em') return parent * n;
  if (u === 'rem') return 16 * n;
  if (u === '%') return (parent * n) / 100;
  return toPx(n, u, { fontSize: parent }) ?? parent;
}

export function lineHeightPx(style: Record<string, string>, fontPx: number): number {
  const raw = (style['line-height'] ?? 'normal').trim();
  if (raw === 'normal') return fontPx * 1.2;
  if (/^[0-9.]+$/.test(raw)) return fontPx * parseFloat(raw);
  const m = /^(-?[0-9.]+)(px|pt|em|rem|%)?$/.exec(raw);
  if (!m) return fontPx * 1.2;
  if (!m[2]) return fontPx * parseFloat(m[1]);
  if (m[2] === '%') return (fontPx * parseFloat(m[1])) / 100;
...
export function measureWidth(text: string, style: Record<string, string>): number {
...
function charFactor(ch: string): number {
  ...
  if (code > 0x2500) return 0.7; // box drawing / CJK-ish wide fallback
  if (code > 0x7f) return 0.55;
  return 0.5;
}

function lengthPx(raw: string, fontPx: number): number {
