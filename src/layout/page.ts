...
const SIZE_TABLE: Record<string, [number, number]> = {
  a5: [148, 210], a4: [210, 297], a3: [297, 420],
  letter: [215.9, 279.4], legal: [215.9, 355.6],
};

export function pageBoxFromRules(rules: PageRule[]): PageBox {
  const decl: Record<string, string> = {};
  for (const r of rules) {
    if (r.name === 'default' && !r.pseudo) Object.assign(decl, r.declarations);
  }
  ...
  try {
    ...
    if (
      Number.isFinite(parsed.wMm) && parsed.wMm > 0 &&
    ...
    if (named) { wMm = named[0]; hMm = named[1]; }
    if (parts.includes('landscape')) { const t = wMm; wMm = hMm; hMm = t; }
  }
...
function parseMarginShorthand(shorthand: string, decl: Record<string, string>): [number, number, number, number] {
  const get = (side: string, fallback: string): number => {
  ...
  if (parts.length === 0) { t = r = b = l = '0'; }
  else if (parts.length === 1) { t = r = b = l = parts[0]; }
  else if (parts.length === 2) { [t, r] = parts; b = t; l = r; }
  else if (parts.length === 3) { [t, r, b] = parts; l = r; }
  else { [t, r, b, l] = parts; }
  return [get('top', t), get('right', r), get('bottom', b), get('left', l)];
}

function lengthPx(raw: string, fontSize: number): number {
