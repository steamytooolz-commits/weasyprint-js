...
function splitTrackList(input: string): string[] {
  const out: string[] = [];
  let depth = 0;
  let cur = '';
...
function expandRepeats(spec: string): string[] {
...
function parseFixedPx(token: string, containerPx: number): number | null {
...
function parseFr(token: string): number | null {
  const t = token.trim().toLowerCase();
  if (t === 'auto') return null;
  const m = /^(-?[0-9]*\.?[0-9]+)?fr$/.exec(t);
  if (!m) return null;
  if (m[1] == null || m[1] === '') return 1;
  const n = parseFloat(m[1]);
...
function isAutoToken(token: string): boolean {
  const t = token.trim().toLowerCase();
  if (t === 'auto' || t === 'min-content' || t === 'max-content') return true;
  if (t.indexOf('minmax(') === 0 || t.indexOf('fit-content(') === 0) return true;
  return false;
}

function minmaxAsFr(token: string): number | null {
  const t = token.trim().toLowerCase();
  const m = /^minmax\(\s*([^,]+)\s*,\s*([^)]+)\s*\)$/.exec(t);
  if (!m) return null;
  const maxFr = parseFr(m[2].trim());
  if (maxFr != null) return maxFr;
  return null;
}

export function layoutGridTracks(
  spec: string,
  containerPx: number,
  gap: number,
  count: number,
  ...
  if (autoM) {
    ...
    if (mm) {
      ...
      if (p != null && p > 0) minPx = p;
    } else {
      const p = parseFixedPx(inner, container);
      if (p != null && p > 0) minPx = p;
    }
    let fit = itemCount;
    if (Number.isFinite(minPx) && (minPx as number) > 0) {
      fit = Math.max(1, Math.floor((container + gapPx) / ((minPx as number) + gapPx)));
      if (itemCount > 0) fit = Math.min(fit, itemCount);
      if (!Number.isFinite(fit) || fit < 1) fit = 1;
    } else if (fit <= 0) { return []; }
  ...
  const frs: number[] = new Array(n).fill(0);
  const isAuto: boolean[] = new Array(n).fill(false);
  for (let i = 0; i < n; i++) {
    const tok = defList[i];
    const fr = parseFr(tok);
    if (fr != null) { frs[i] = fr; continue; }
    const mmFr = minmaxAsFr(tok);
    if (mmFr != null) { frs[i] = mmFr; continue; }
    const fx = parseFixedPx(tok, container);
    if (fx != null) { fixed[i] = Math.max(0, fx); continue; }
    if (tok.indexOf('minmax(') === 0) {
      const m = /^minmax\(\s*([^,]+)\s*,\s*([^)]+)\s*\)$/.exec(tok);
      if (m) {
        const mx = parseFixedPx(m[2].trim(), container);
        if (mx != null) { fixed[i] = Math.max(0, mx); continue; }
      }
      isAuto[i] = true; continue;
    }
    isAuto[i] = true;
  }
  void isAutoToken;
  ...
  let fixedSum = 0;
  for (let i = 0; i < n; i++) if (fixed[i] != null) fixedSum += fixed[i] as number;
  ...
  if (totalUnits > 0) {
    ...
    for (let i = 0; i < n; i++) {
      if (fixed[i] != null) out[i] = fixed[i] as number;
      else if (frs[i] > 0) out[i] = unit * frs[i];
      else out[i] = unit;
    }
  } else {
    for (let i = 0; i < n; i++) out[i] = fixed[i] as number;
  }
  return out;
}

function parseLinePart(part: string): { span: boolean; value: number | null } {
  const t = part.trim().toLowerCase();
  if (t === '' || t === 'auto') return { span: false, value: null };
  ...
  if (Number.isFinite(nn)) return { span: false, value: nn };
  return { span: false, value: null };
}

function resolveAxis(
  shorthand: string | undefined,
  startLong: string | undefined,
  endLong: string | undefined,
  defStart: number,
): { start: number; end: number } {
  let start: number | null = null;
  let end: number | null = null;
  let startSpan: number | null = null;
  let endSpan: number | null = null;
  if (shorthand != null && shorthand.trim() !== '') {
    const parts = shorthand.split('/').map((p) => p.trim());
    if (parts.length === 1) {
      const p = parseLinePart(parts[0]);
      if (p.span) startSpan = p.value ?? 1;
      else if (p.value != null) start = p.value;
    } else {
      const a = parseLinePart(parts[0] ?? '');
      const b = parseLinePart(parts[1] ?? '');
      if (a.span) startSpan = a.value ?? 1;
      else if (a.value != null) start = a.value;
      if (b.span) endSpan = b.value ?? 1;
      else if (b.value != null) end = b.value;
    }
  }
  if (startLong != null && startLong.trim() !== '') {
    const p = parseLinePart(startLong);
    if (p.span) { start = null; startSpan = p.value ?? 1; }
    else { start = p.value; startSpan = null; }
  }
  if (endLong != null && endLong.trim() !== '') {
    const p = parseLinePart(endLong);
    if (p.span) { end = null; endSpan = p.value ?? 1; }
    else { end = p.value; endSpan = null; }
  }
  let s = start ?? defStart;
  let e: number | null = end;
  if (e == null) {
    if (endSpan != null) e = s + endSpan;
    else if (startSpan != null) { s = start ?? defStart; e = s + (startSpan ?? 1); }
    else e = s + 1;
  } else if (start == null && startSpan != null) {
    s = e - (startSpan ?? 1);
  }
  if (!Number.isFinite(s)) s = defStart;
  if (!Number.isFinite(e)) e = s + 1;
  let si = Math.max(1, Math.floor(s));
  let ei = Math.max(si + 1, Math.floor(e as number));
  if ((start ?? 0) < 0 || (end ?? 0) < 0) {
    if ((end ?? 0) < 0 && (start ?? 0) >= 0) ei = si + 1;
    else if ((start ?? 0) < 0 && (end ?? 0) >= 0) si = Math.max(1, ei - 1);
  }
  return { start: si, end: ei };
}

export function parseGridPlacement(style: Record<string, string>): GridPlacement {
  const get = (k: string): string | undefined => style[k];
  ...
  if (area != null && area.trim() !== '' && area.trim().toLowerCase() !== 'auto') {
    ...
    if (parts.length === 4) {
      ...
      rowShorthand = parts[0]; colShorthand = parts[1];
