...
export class FloatManager {
  ...
  private readonly floats: FloatBox[] = [];
  ...
  addFloat(side: FloatSide, y: number, w: number, h: number): void {
    const s: FloatSide = side === 'right' ? 'right' : 'left';
  ...
  private overlaps(f: FloatBox, y: number, h: number): boolean {
   ...
   */
  availableWidthAt(y: number, h: number): { x: number; width: number } {
    const yy = Number.isFinite(y) ? y : 0;
    const hh = Number.isFinite(h) ? h : 0;
    let left = 0;
    let right = 0;
    for (const f of this.floats) {
      if (!this.overlaps(f, yy, hh)) continue;
      if (f.side === 'left') left = Math.max(left, f.w);
      else right = Math.max(right, f.w);
    }
  ...
  clearY(clear: string, y: number): number {
    ...
    if (!wantsLeft && !wantsRight) return res;
    for (const f of this.floats) {
      const bottom = f.y + f.h;
      if (bottom <= res) continue;
      if (f.side === 'left' && wantsLeft) res = Math.max(res, bottom);
      if (f.side === 'right' && wantsRight) res = Math.max(res, bottom);
    }
    return res;
  }
}
