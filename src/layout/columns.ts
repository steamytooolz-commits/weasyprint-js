...
function parseLengthToPx(raw: string, reference: number): number | null {
...
function parseGap(style: Record<string, string>, reference: number): number {
...
export function columnCount(width: number, style: Record<string, string>): number {
    ...
    if (shorthand !== '' && shorthand !== 'auto') {
      ...
      for (const p of parts) {
        ...
        if (/^[0-9]+$/.test(p)) {
          ...
          if (Number.isFinite(n) && n >= 1 && countExplicit == null) countExplicit = n;
          continue;
        }
        const px = parseLengthToPx(p, w);
        if (px != null && px > 0 && widthExplicit == null) widthExplicit = px;
      }
    }
  }
  ...
  if (countExplicit != null) return Math.max(1, countExplicit);
...
export function columnWidth(count: number, containerPx: number, gapPx: number): number {
  ...
  if (c <= 0) return Math.max(0, container);
  if (c === 1) return Math.max(0, container);
 ...
 */
export function distributeToColumns<T>(items: T[], heights: number[], count: number): T[][] {
  const c = Number.isFinite(count) ? Math.floor(count) : 1;
  if (c <= 0) return [items.slice()];
  const cols: T[][] = Array.from({ length: c }, () => []);
  if (items.length === 0) return cols;
  const colHeights: number[] = new Array(c).fill(0);
  for (let i = 0; i < items.length; i++) {
    let best = 0;
    for (let k = 1; k < c; k++) {
      if (colHeights[k] < colHeights[best]) best = k;
    }
