...
function clampSize(v: number, minPx: number, maxPx: number): number {
  const lo = Number.isFinite(minPx) ? Math.max(0, minPx) : 0;
  const hi = Number.isFinite(maxPx) ? maxPx : Infinity;
  const hiFixed = hi < lo ? lo : hi;
  if (v < lo) return lo;
  if (v > hiFixed) return hiFixed;
  return v;
}

function sanitise(n: number, fallback: number): number {
...
export function layoutFlexRow(
  ...
  const grow: number[] = new Array(n);
  const shrink: number[] = new Array(n);
  const minPx: number[] = new Array(n);
  const maxPx: number[] = new Array(n);
  for (let i = 0; i < n; i++) {
    const it = items[i];
  ...
  const hyp: number[] = new Array(n);
  for (let i = 0; i < n; i++) hyp[i] = clampSize(basis[i], minPx[i], maxPx[i]);
  const sizes: number[] = hyp.slice();
  ...
  if (free > 0) {
    ...
    let unfrozen = new Set<number>();
    for (let i = 0; i < n; i++) {
      if (grow[i] > 0 && sizes[i] < maxPx[i]) unfrozen.add(i);
    }
    let remaining = free;
    // Items with no grow stay at hyp.
    for (let iter = 0; iter <= n && unfrozen.size > 0; iter++) {
      let totalGrow = 0;
      for (const i of unfrozen) totalGrow += grow[i];
      ...
      for (const i of unfrozen) {
        ...
        if (p < minPx[i] - 1e-9 || p > maxPx[i] + 1e-9) violating.push(i);
      }
      if (violating.length === 0) {
        for (const i of unfrozen) sizes[i] = proposals.get(i) ?? sizes[i];
        break;
      }
      ...
      if (violating.length === unfrozen.size + violating.length) break;
    }
  } else if (free < 0) {
    ...
    let unfrozen = new Set<number>();
    for (let i = 0; i < n; i++) {
      if (shrink[i] > 0 && sizes[i] > minPx[i]) unfrozen.add(i);
    }
    let remainingDeficit = deficit0;
    for (let iter = 0; iter <= n && unfrozen.size > 0; iter++) {
      let totalScaled = 0;
      for (const i of unfrozen) totalScaled += shrink[i] * hyp[i];
      if (totalScaled <= 0) {
  ...
  for (let i = 0; i < n; i++) {
    if (!Number.isFinite(sizes[i])) sizes[i] = hyp[i];
