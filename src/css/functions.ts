// CSS functions A — attr/counter/counters/target-counter parsing.
import type { CounterScopes } from './counters.js';
import type { TargetCollector } from './targets.js';
export interface AttrRef { name: string; fallback: string | null }
export interface CounterRef { name: string; style: string }
export interface CountersRef { name: string; separator: string; style: string }
export interface TargetCounterRef { target: string; counter: string; style: string }
...
export function parseAttr(raw: string): AttrRef | null {
...
export function parseCounter(raw: string): CounterRef | null {
...
export function parseCounters(raw: string): CountersRef | null {
...
export function parseTargetCounter(raw: string): TargetCounterRef | null {
...
export function resolveTargetId(rawTarget: string, ctx: ContentContext): string {
  ...
  if (urlM) return unquote(urlM[1].trim());
  return unquote(t);
}
export type ContentToken =
  | { kind: 'string'; value: string }
  | { kind: 'url'; value: string }
  | { kind: 'counter'; raw: string }
  | { kind: 'counters'; raw: string }
  | { kind: 'target-counter'; raw: string }
  | { kind: 'attr'; raw: string }
  | { kind: 'open-quote' } | { kind: 'close-quote' }
  | { kind: 'no-open-quote' } | { kind: 'no-close-quote' };
/** Split a `content` value into typed tokens. Returns null if invalid. */
export function tokenizeContent(value: string): ContentToken[] | null {
  const v = value.trim();
  if (/^(normal|none)$/i.test(v)) return [];
  ...
  while (i < v.length) {
    ...
    if (kw) { tokens.push({ kind: kw[1].toLowerCase() } as ContentToken); i += kw[1].length; continue; }
    if (rest[0] === '"' || rest[0] === "'") {
      const q = rest[0];
      let j = 1;
    ...
    if (fn) {
      ...
      } else if (name === 'attr') tokens.push({ kind: 'attr', raw });
      else if (name === 'counter') tokens.push({ kind: 'counter', raw });
      else if (name === 'counters') tokens.push({ kind: 'counters', raw });
      else tokens.push({ kind: 'target-counter', raw });
      i += j + 1; continue;
    }
    return null;
  }
...
export function resolveAttr(raw: string, attrs: Record<string, string>): string {
...
function quoteAt(quotes: string[] | undefined, depth: number, open: boolean): string {
...
export function resolveContentList(
  style: Record<string, string>,
  context: ContentContext = {},
): string {
  const raw = (style['content'] ?? 'normal').trim();
  if (/^(normal|none)$/i.test(raw) || raw === '') return '';
  ...
  for (const t of tokens) {
    switch (t.kind) {
      ...
      case 'url': out += t.value; break;
      case 'attr': out += resolveAttr(t.raw, attrs); break;
      case 'counter': {
      ...
      case 'open-quote': out += quoteAt(context.quotes, depth, true); depth += 1; break;
      case 'close-quote': depth = Math.max(0, depth - 1); out += quoteAt(context.quotes, depth, false); break;
      case 'no-open-quote': depth += 1; break;
      case 'no-close-quote': depth = Math.max(0, depth - 1); break;
    }
  }
  return out;
}
