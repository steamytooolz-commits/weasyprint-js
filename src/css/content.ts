// Generated content — mirrors weasyprint/css/content.py content resolution
// (strings, counter()/counters(), attr(), quotes) for ::before/::after.
...
function unquoteString(tok: string): string | null {
  const q = tok[0];
...
export function resolveContent(
  ...
  const v = value.trim();
  if (!v || /^(normal|none)$/i.test(v)) return null;
  ...
  for (const tok of tokenize(v)) {
    ...
    if (low === 'no-open-quote') { depth++; continue; }
    if (low === 'no-close-quote') { depth = Math.max(0, depth - 1); continue; }
    if (tok[0] === '"' || tok[0] === "'") {
    ...
    if (m) {
      ...
      if (got != null) { out += got; any = true; }
      else if (args[1] != null) {
        const fb = unquoteString(args[1]);
        if (fb != null) { out += fb; any = true; }
      }
      continue;
    }
    // url(), target-counter(), target-text(), leader(): unsupported → skip.
    if (/^(url|target-counter|target-text|leader)\(/i.test(tok)) continue;
...
function formatPageCounter(n: number, style: string): string {
  const s = style.trim().toLowerCase() || 'decimal';
  if (s === 'decimal-leading-zero') return String(n).padStart(2, '0');
  if (s === 'lower-roman') return toRoman(n).toLowerCase();
  if (s === 'upper-roman') return toRoman(n);
  return String(n);
}
...
export function parseQuotes(value: string | undefined): string[] {
  if (!value) return ['“', '”', '‘', '’'];
