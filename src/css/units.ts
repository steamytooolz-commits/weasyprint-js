...
export function inchToPxinch(inch: number): number {
  return inch * 96;
}
...
export function toPx(value: number, unit: string | null, opts: {
  fontSize?: number; rootFontSize?: number; dpi?: number;
} = {}): number | null {
  if (unit == null) return value;
  const u = unit.toLowerCase();
  if (u in ABSOLUTE) return value * ABSOLUTE[u];
  if (u === 'em' || u === 'rem' || u === 'ex' || u === 'ch' || u === 'cap') {
    const base = u === 'rem' ? (opts.rootFontSize ?? 16) : (opts.fontSize ?? 16);
    if (u === 'em' || u === 'rem') return value * base;
    return value * base * 0.5; // approx ex/ch/cap
  }
  if (u === '%') return null; // handled by caller (percent of containing block)
  return null; // unknown / viewport units resolved in layout
}

export function parseLength(
  text: string, opts?: { fontSize?: number; rootFontSize?: number },
