...
const BORDER_WIDTHS: ReadonlySet<string> = new Set(['thin', 'medium', 'thick']);
...
const PAGE_BREAKS: ReadonlySet<string> = new Set([
...
export function isValidColor(v: string): boolean {
  ...
  if (/^(rgb|rgba|hsl|hsla|hwb|lab|lch|color)\(/.test(s) && s.endsWith(')')) return true;
  return false;
}
export function normalizeColor(v: string): string | null {
  const t = v.trim();
  if (!isValidColor(t)) return null;
  if (/^#/.test(t)) return t.toLowerCase();
  if (NAMED_COLORS.has(t.toLowerCase())) return t.toLowerCase();
  return t.replace(/\s+/g, ' ').trim();
}
export function isValidLength(v: string): boolean {
...
export function isValidLengthOrPct(v: string): boolean {
  const t = v.trim().toLowerCase();
  if (isValidLength(t)) return true;
  return /^[+-]?(\d*\.?\d+)%$/.test(t);
}
export function normalizeLength(v: string): string | null {
  const t = v.trim().toLowerCase();
  if (/^[+-]?(\d*\.?\d+)$/.test(t)) {
    if (parseFloat(t) !== 0) return null;
    return '0px';
  }
  const m = /^([+-]?(?:\d*\.?\d+))([a-z%]+)$/.exec(t);
  if (!m) return null;
  if (m[2] === '%' || !LENGTH_UNITS.has(m[2])) return null;
...
const FONT_SIZES: ReadonlySet<string> = new Set([
...
const LIST_STYLES: ReadonlySet<string> = new Set([
  'none', 'disc', 'circle', 'square', 'decimal', 'decimal-leading-zero',
  'lower-roman', 'upper-roman', 'lower-alpha', 'upper-alpha',
  'lower-latin', 'upper-latin', 'lower-greek', 'armenian', 'georgian',
]);
const LIST_POSITIONS: ReadonlySet<string> = new Set(['inside', 'outside']);
function validFontFamily(v: string): string | null {
  const t = v.trim();
  if (!t) return null;
  const parts = t.split(',');
  const norm: string[] = [];
...
function validFontSize(v: string): string | null {
  const t = v.trim().toLowerCase();
  if (FONT_SIZES.has(t)) return t;
  if (isValidLengthOrPct(v.trim())) return v.trim().toLowerCase();
  return null;
}
function splitBorderShorthand(v: string): { w: string | null; s: string | null; c: string | null } {
  const parts = v.trim().split(/\s+/).filter(Boolean);
  if (parts.length === 0 || parts.length > 3) return { w: null, s: null, c: null };
  let w: string | null = null;
  let s: string | null = null;
  let c: string | null = null;
  for (const p of parts) {
    const pl = p.toLowerCase();
    if (!w && (BORDER_WIDTHS.has(pl) || isValidLength(p))) w = BORDER_WIDTHS.has(pl) ? pl : normalizeLength(p);
    else if (!s && BORDER_STYLES.has(pl)) s = pl;
    else if (!c && isValidColor(p)) c = normalizeColor(p);
    else return { w: null, s: null, c: null };
  }
  return { w, s, c };
}
function validBorderSide(kind: 'width' | 'style' | 'color', v: string): string | null {
  const t = v.trim();
  if (kind === 'width') {
    if (BORDER_WIDTHS.has(t.toLowerCase())) return t.toLowerCase();
    return normalizeLength(t);
  }
  if (kind === 'style') return BORDER_STYLES.has(t.toLowerCase()) ? t.toLowerCase() : null;
  return normalizeColor(t);
}
function validBackground(v: string): string | null {
  const t = v.trim();
  if (/^(none|transparent)$/i.test(t)) return t.toLowerCase();
  return normalizeColor(t) ?? (/^url\(/i.test(t) ? t.replace(/\s+/g, ' ') : null);
}
function validContent(v: string): string | null {
  const t = v.trim();
  if (/^(normal|none)$/i.test(t)) return t.toLowerCase();
  let i = 0;
  const n = t.length;
  let found = false;
  while (i < n) {
    ...
    if (fn) {
      ...
      for (; j < rest.length; j++) {
        const ch = rest[j];
        if (quote) { if (ch === quote && rest[j - 1] !== '\\') quote = null; continue; }
...
function validQuotes(v: string): string | null {
  const t = v.trim().toLowerCase();
  if (t === 'none' || t === 'auto') return t;
  const parts: string[] = [];
  let i = 0;
  const s = v.trim();
  while (i < s.length) {
...
function validListStyle(v: string): string | null {
  const t = v.trim().toLowerCase();
  ...
  for (const p of parts) {
    const pl = p.toLowerCase();
    if (!type && LIST_STYLES.has(pl)) type = pl;
    else if (!pos && LIST_POSITIONS.has(pl)) pos = pl;
    else if (!/^url\(/i.test(p)) return null;
  }
  if (!type && !pos) return null;
  return [type, pos].filter(Boolean).join(' ');
}
function validFlexGrid(v: string, prop: string): string | null {
  const t = v.trim().toLowerCase();
  ...
  if (prop === 'flex-basis') {
    if (t === 'auto' || t === 'content') return t;
    if (isValidLengthOrPct(v.trim())) return v.trim().toLowerCase();
    return null;
  }
  if (prop === 'flex') {
    if (t === 'none') return '0 0 auto';
    if (t === 'auto') return '1 1 auto';
    if (/^\d*\.?\d+$/.test(t)) return `${parseFloat(t)} 1 0%`;
    if (isValidLengthOrPct(v.trim())) return `1 1 ${v.trim().toLowerCase()}`;
    return null;
  }
...
function validBackgroundImage(v: string): string | null {
  const t = v.trim();
  if (/^none$/i.test(t)) return 'none';
  // Single url() layer (multiple layers unsupported — first wins downstream).
  const m = /url\(\s*(['"]?)(.*?)\1\s*\)/i.exec(t);
  if (m && m[2].trim()) return `url(${m[2].trim()})`;
  // Gradients degrade: not painted, but valid so fallbacks still cascade.
  if (/^(linear|radial|repeating-linear|repeating-radial|conic)-gradient\(/i.test(t)) return t;
  return null;
}

function validBoxShadow(v: string): string | null {
  const t = v.trim();
  if (/^none$/i.test(t)) return 'none';
  ...
  for (const part of parts) {
    if (part.toLowerCase() === 'inset') { inset = true; continue; }
    rest.push(part);
  }
...
export function validateDeclaration(prop: string, value: string): string | null {
  const p = prop.trim().toLowerCase();
  const v = value.trim();
  if (/^--/.test(p)) return v;
  if (!p || !v || !SUPPORTED_PROPERTIES.has(p)) return null;
  ...
  if (p === 'box-shadow') return validBoxShadow(v);
  if (p === 'background-image') return validBackgroundImage(v);
  ...
  if (p === 'display') return DISPLAY_VALUES.has(v.toLowerCase()) ? v.toLowerCase() : null;
  if (p === 'font-family') return validFontFamily(v);
  if (p === 'font-size') return validFontSize(v);
  if (p === 'font-weight') {
    const t = v.toLowerCase();
  ...
  if (p === 'font-style') {
    const t = v.toLowerCase();
    if (FONT_STYLES.has(t)) return t;
    return /^oblique\s+[-+]?\d+deg$/i.test(v) ? v.toLowerCase() : null;
  }
  if (p === 'text-align') return TEXT_ALIGNS.has(v.toLowerCase()) ? v.toLowerCase() : null;
  ...
  if (p === 'content') return validContent(v);
  if (p === 'quotes') return validQuotes(v);
  if (p === 'list-style-type') return LIST_STYLES.has(v.toLowerCase()) ? v.toLowerCase() : null;
  if (p === 'list-style-position') return LIST_POSITIONS.has(v.toLowerCase()) ? v.toLowerCase() : null;
  if (p === 'list-style') return validListStyle(v);
  if (/^(margin|padding)-(top|right|bottom|left)$/.test(p)) {
    if (v.toLowerCase() === 'auto' && p.startsWith('margin')) return 'auto';
    return isValidLengthOrPct(v) ? v.toLowerCase() : null;
  }
  if (/^(width|height|min-width|text-indent|letter-spacing)$/.test(p)) {
    const t = v.toLowerCase();
    if (['auto', 'normal', 'none'].includes(t)) return t;
    return isValidLengthOrPct(v) ? v.toLowerCase() : null;
  }
  if (p === 'line-height') {
    const t = v.toLowerCase();
    ...
    if (isValidLengthOrPct(v)) return v.toLowerCase();
    return null;
  }
  ...
  if (flexGrid.includes(p)) return validFlexGrid(v, p);
  if (p === 'border' || p === 'border-top' || p === 'outline') {
    const parts = splitBorderShorthand(v);
    if (!parts.w && !parts.s && !parts.c) return null;
  ...
  if (/^border-(top|right|bottom|left)-(width|style|color)$/.test(p)) {
    ...
    return validBorderSide(kind as 'width' | 'style' | 'color', v);
  }
  ...
  if (p === 'border-spacing') {
    ...
    if (!parts.every((x) => isValidLength(x))) return null;
    return parts.map((x) => x.toLowerCase()).join(' ');
  }
  if (p === 'background') return validBackground(v);
  return v.replace(/\s+/g, ' ').trim() || null;
}
export const SUPPORTED_PROPERTIES: Set<string> = new Set([
