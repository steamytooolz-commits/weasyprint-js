...
export function buildSiblingCtx(elements: ElementRef[]): SiblingCtx {
...
export function specificity(selector: string): [number, number, number] {
  ...
  work = work.replace(/:not\(([^()]*)\)/g, (_m, inner: string) => {
  ...
  for (const p of noPseudo.split(/[\s>+~]+/)) {
    ...
    if (/^[A-Za-z][A-Za-z0-9-]*/.exec(p)) c += 1;
  }
  return [a, b, c];
}
...
interface SelPart { compound: string; combinator: ' ' | '>' | '+' | '~' | null }

/** Split a selector into compounds with combinators (left→right). */
function tokenize(sel: string): SelPart[] {
  const parts: SelPart[] = [];
  let depth = 0;
  let bracket = false;
  let cur = '';
  ...
  const flush = (): void => {
    const t = cur.trim();
    if (t) parts.push({ compound: t, combinator: comb });
    cur = '';
    comb = null;
  };
  for (let i = 0; i < sel.length; i++) {
    const ch = sel[i];
    if (ch === '(' && !bracket) { depth++; cur += ch; continue; }
    if (ch === ')' && !bracket) { depth = Math.max(0, depth - 1); cur += ch; continue; }
    if (ch === '[') { bracket = true; cur += ch; continue; }
    if (ch === ']') { bracket = false; cur += ch; continue; }
...
export function matches(
  ...
  const ctx: SiblingCtx | null = sibCtx ?? (byUid
    ? { byUid, childrenOf: new Map<number | null, number[]>(), indexOf: new Map<number, number>() }
    : null);
  let cur: ElementRef | undefined = el;
  for (let i = parts.length - 1; i >= 0; i--) {
    if (!cur || !matchesCompound(cur, parts[i].compound, ctx)) return false;
    if (i === 0) return true;
    ...
    if (comb === '>') {
      ...
      let found: ElementRef | undefined;
      while (anc) {
        if (matchesCompound(anc, parts[i - 1].compound, ctx)) { found = anc; break; }
      ...
      if (!ctx) return false;
      const sibs: number[] = ctx.childrenOf.get(cur.parentUid) ?? [];
      const idx: number = ctx.indexOf.get(cur.uid) ?? -1;
      if (comb === '+') {
        const prev: ElementRef | undefined = idx > 0 ? ctx.byUid.get(sibs[idx - 1]) : undefined;
        if (!prev || !matchesCompound(prev, parts[i - 1].compound, ctx)) return false;
        cur = prev;
        continue;
      }
      let found: ElementRef | undefined;
      for (let k = idx - 1; k >= 0; k--) {
        ...
        if (cand && matchesCompound(cand, parts[i - 1].compound, ctx)) { found = cand; break; }
      }
      if (!found) return false;
      cur = found;
      continue;
...
export function matchesCompound(
  el: ElementRef,
  compound: string,
  ctx?: SiblingCtx | { byUid: Map<number, ElementRef>; childrenOf: Map<number | null, number[]>; indexOf: Map<number, number> } | null,
): boolean {
  ...
  for (const inner of nots) {
    ...
    for (const alt of alts) {
      if (matchesCompound(el, alt, ctx)) return false;
    }
  }
  // Other pseudo-classes.
  const pseudos: string[] = [];
  work = work.replace(/:([a-zA-Z-]+)(\([^()]*\))?/g, (_m, name: string, args?: string) => {
  ...
  for (const m of work.matchAll(re)) {
    ...
    const op = m[2]; const want = m[3] ?? '';
    if (op === '=' && got !== want) return false;
    if (op === '~=' && !got.split(/\s+/).includes(want)) return false;
    if (op === '|=' && !(got === want || got.startsWith(want + '-'))) return false;
    if (op === '^=' && !got.startsWith(want)) return false;
    if (op === '$=' && !got.endsWith(want)) return false;
    if (op === '*=' && !got.includes(want)) return false;
  }
  for (const pseudo of pseudos) {
    if (!evalPseudo(el, pseudo, ctx ?? null)) return false;
  }
  return true;
}
...
function evalPseudo(
  el: ElementRef,
  pseudo: string,
  ctx: { byUid: Map<number, ElementRef>; childrenOf: Map<number | null, number[]>; indexOf: Map<number, number> } | null,
): boolean {
  ...
  switch (name) {
    ...
    case 'empty': {
      ...
      if (kids != null) return kids.length === 0;
      return true;
    }
    case 'link':
    case 'any-link':
...
function sibIndex(
  el: ElementRef,
  ctx: { indexOf: Map<number, number> } | null,
): number {
...
function siblingsOf(
  el: ElementRef,
  ctx: { byUid: Map<number, ElementRef>; childrenOf: Map<number | null, number[]> } | null,
): ElementRef[] {
  if (!ctx) return [el];
  const uids = ctx.childrenOf.get(el.parentUid) ?? [el.uid];
  return uids.map((u) => ctx.byUid.get(u)).filter((e): e is ElementRef => e != null);
}
...
export function nth(pos: number, expr: string): boolean {
  const e = expr.trim().toLowerCase();
  if (e === 'odd') return pos % 2 === 1;
  if (e === 'even') return pos % 2 === 0;
  if (/^[-+]?\d+$/.test(e)) return pos === parseInt(e, 10);
