...
export interface CascadeResult {
  ...
  pseudos: { before: Map<number, Map<string, CascadeEntry>>; after: Map<number, Map<string, CascadeEntry>> };
}
...
export function buildCascade(
  elements: ElementRef[],
  sheets: Array<{ css: string; origin: Origin; media?: string | null }>,
  opts: { mediaType?: string; presentationalHints?: boolean } = {},
): CascadeResult {
  const mediaType = opts.mediaType ?? 'print';
  const matched = new Map<number, Map<string, CascadeEntry>>();
  const origins = new Map<string, Origin>();
  for (const el of elements) matched.set(el.uid, new Map());
  const byUid = new Map(elements.map((e) => [e.uid, e]));
  const sibCtx = buildSiblingCtx(elements);
  const pseudos: CascadeResult['pseudos'] = { before: new Map(), after: new Map() };
  const pageRules: PageRule[] = [];
  const fontFaces: FontFaceRule[] = [];
  const all = [
    { css: UA_STYLESHEET, origin: 'ua' as Origin },
    { css: UA_FORM_STYLESHEET, origin: 'ua' as Origin },
    { css: PH_STYLESHEET, origin: 'ua' as Origin },
    ...sheets,
  ];

  const applyRules = (
    rules: Array<{ selectors: string[]; declarations: { prop: string; value: string; important: boolean }[]; media: string | null; order: number }>,
    origin: Origin,
  ): void => {
    for (const rule of rules) {
      if (rule.media && !evaluateMedia(rule.media, mediaType)) continue;
        ...
        for (const el of elements) {
          ...
          if (pseudoKind === 'before' || pseudoKind === 'after') {
            ...
            for (const d of decls) {
              ...
              const next: CascadeEntry = {
                value: validated, important: d.important, specificity: spec, order: rule.order,
              };
              if (!prev || wins(next, origin, prev, origin)) pm.set(d.prop, next);
            }
            continue;
          }
  ...
  for (const s of all) {
    if (s.media && !evaluateMedia(s.media, mediaType)) continue;
    const sheet = parseStylesheet(s.css, s.media ?? null);
    const styles = sheet.rules.filter((r) => r.type === 'style');
    applyRules(styles as never, s.origin);
    for (const r of sheet.rules) {
      if (r.type !== 'at') continue;
      if (r.name === 'page') {
        ...
        if (!evaluateMedia(r.prelude, mediaType)) continue;
        const inner = parseStylesheet(r.block ?? '');
        applyRules(inner.rules.filter((x) => x.type === 'style') as never, s.origin);
      }
    }
  }
...
function trailingPseudo(sel: string): 'before' | 'after' | null {
  const m = /::?(before|after)(\([^()]*\))?\s*$/i.exec(sel.trim());
  if (!m) return null;
  return m[1].toLowerCase() as 'before' | 'after';
}

const RANK: Record<string, (imp: boolean) => number> = {
  ua: () => 0,
  user: (imp) => (imp ? 4 : 1),
  author: (imp) => (imp ? 3 : 2),
};

function wins(next: CascadeEntry, no: Origin, prev: CascadeEntry, po: Origin): boolean {
  const rn = RANK[no](next.important);
  const rp = RANK[po](prev.important);
  if (rn !== rp) return rn > rp;
...
export function parsePageRule(prelude: string, block: string): PageRule {
...
function splitPageBlock(block: string): {
  ...
  const cleaned = block.replace(/@([\w-]+)\s*\{([^{}]*)\}/g, (_m, name: string, body: string) => {
...
export function declMap(block: string): Record<string, string> {
...
function applyHints(
  ...
  for (const el of elements) {
    ...
    if (tag === 'body' && a['text']) setHint(matched, origins, el, 'color', a['text']);
    if (a['color'] && tag === 'font') setHint(matched, origins, el, 'color', a['color']);
    if (a['face'] && tag === 'font') setHint(matched, origins, el, 'font-family', a['face']);
    if (a['size'] && tag === 'font') {
      ...
      if (mapped) setHint(matched, origins, el, 'font-size', mapped);
    }
    if (a['align']) {
    ...
    if (a['border'] && (tag === 'table' || tag === 'img')) {
      ...
      if (/^\d+$/.test(b)) setHint(matched, origins, el, 'border-width', `${b}px`);
    }
    if (a['cellspacing'] && tag === 'table') {
      const cs = a['cellspacing'].trim();
      if (/^\d+$/.test(cs)) setHint(matched, origins, el, 'border-spacing', `${cs}px`);
    }
    ...
    if (tag === 'ol' || tag === 'ul') {
      if (a['type']) {
        ...
        if (map[t]) setHint(matched, origins, el, 'list-style-type', map[t]);
      }
    }
    if (tag === 'li' && a['type']) {
      const t = a['type'].trim().toLowerCase();
...
export function computedStyle(
  ...
  elements: ElementRef[],
  uid: number,
  memo = new Map<number, Record<string, string>>(),
): Record<string, string> {
  if (memo.has(uid)) return memo.get(uid)!;
  const el = elements.find((e) => e.uid === uid)!;
  const out: Record<string, string> = {};
  for (const [prop, entry] of matched.get(uid) ?? new Map()) out[prop] = entry.value;
  if (el.parentUid != null) {
    ...
    for (const prop of Object.keys(parent)) {
      ...
      if (!(prop in out) && (prop.startsWith('--') || isInherited(prop))) out[prop] = parent[prop];
    }
  }
  for (const prop of INHERIT_FALLBACK) {
    if (!(prop in out)) out[prop] = initialValue(prop);
  }
 ...
 */
export function computedPseudoStyle(
  ...
  if (!decls || decls.size === 0) return null;
  const out: Record<string, string> = { ...elementStyle };
  for (const [prop, entry] of decls) {
    const computed = computeValue(prop, entry.value, elementStyle);
    if (computed !== '') out[prop] = computed;
  }
  // Pseudo-element boxes are inline by default.
  if (!decls.has('display')) out['display'] = 'inline';
  return out;
}
