// CSS counters — port of weasyprint/css/counters.py (styles + scopes).
export type CounterStyleName = 'decimal' | 'decimal-leading-zero'
  | 'lower-roman' | 'upper-roman' | 'lower-alpha' | 'upper-alpha'
  | 'lower-latin' | 'upper-latin' | 'disc' | 'circle' | 'square'
  | 'none' | (string & {});
export const DISC_SYMBOL = '•';
export const CIRCLE_SYMBOL = '○';
export const SQUARE_SYMBOL = '▪';
export const KNOWN_COUNTER_STYLES: ReadonlySet<string> = new Set([
...
export function formatCounter(value: number, style: string): string {
  ...
  switch (s) {
    ...
    case 'lower-alpha': case 'lower-latin': return toAlpha(n, false) ?? String(n);
    case 'upper-alpha': case 'upper-latin': return toAlpha(n, true) ?? String(n);
...
function toRoman(n: number, upper: boolean): string | null {
  if (!Number.isFinite(n) || n < 1 || n > 3999) return null;
  let rest = n;
  let out = '';
...
function toAlpha(n: number, upper: boolean): string | null {
...
export interface CounterSpec { name: string; value: number }
/** Parse `counter-reset` / `counter-increment` value lists. */
export function parseCounterList(value: string, def: number): CounterSpec[] | null {
  const v = value.trim();
  if (!v) return null;
...
export class CounterScopes {
  ...
  private owners = new Map<string, Array<number | null>>();
  reset(name: string, value = 0, owner: number | null = null): void {
    const key = name.toLowerCase();
    let stack = this.stacks.get(key);
    let own = this.owners.get(key);
    if (!stack || !own) {
      stack = []; own = [];
      this.stacks.set(key, stack); this.owners.set(key, own);
    }
    if (own.length > 0 && own[own.length - 1] === owner) {
      stack[stack.length - 1] = value;
    } else { stack.push(value); own.push(owner); }
  }
  increment(name: string, value = 1, owner: number | null = null): void {
    void owner;
    const key = name.toLowerCase();
  ...
  applyReset(value: string, owner: number | null = null): boolean {
    const specs = parseCounterList(value, 0);
    if (specs === null) return false;
    for (const s of specs) this.reset(s.name, s.value, owner);
    return true;
  }
  applyIncrement(value: string, owner: number | null = null): boolean {
    const specs = parseCounterList(value, 1);
    if (specs === null) return false;
    for (const s of specs) this.increment(s.name, s.value, owner);
    return true;
  }
  ...
  clear(): void { this.stacks.clear(); this.owners.clear(); }
}
